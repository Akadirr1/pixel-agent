...
test('normalizes npm pack array and keyed-object output', () => {
  const metadata = { filename: 'akadirr1-pixel-agent-0.1.0.tgz', files: validFiles() };
  assert.equal(normalizePackMetadata([metadata]), metadata);
  assert.equal(normalizePackMetadata({ '@akadirr1/pixel-agent@0.1.0': metadata }), metadata);
});
...
test('validates release tag, ref, repository, and monotonic version', () => {
  const manifest = {
    name: '@akadirr1/pixel-agent',
    version: '0.1.0',
    repository: { url: 'git+https://github.com/Akadirr1/pixel-agent.git' },
  };
  assert.doesNotThrow(() =>
    validateReleaseIdentity({
      manifest,
  ...
  assert.throws(
    () =>
      validateReleaseIdentity({
        manifest: { ...manifest, version: '0.0.2' },
        releaseTag: 'v0.0.2',
